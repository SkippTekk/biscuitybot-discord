...
client.on("messageDelete", async (message) => {
  const dbMessageDelete = await Guild.findOne({
  ...
  if (dbMessageDelete.logChannel === null) {
    ...
    client.channels.cache.get(dbMessageDelete.logChannel).send({
      content: `:regional_indicator_d: Message delete by ${message.author.tag} - \`\`${message.content}\`\``,
    });
  }
});
...
process.on("uncaughtException", (error, source) => {
  ...
  console.log(client.guilds.resolve());
